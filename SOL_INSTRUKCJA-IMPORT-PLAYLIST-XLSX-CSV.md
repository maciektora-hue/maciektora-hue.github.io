...
**Ważne:** w repozytorium nie ma obecnie zatwierdzonego, aktywnego importera playlist w Pythonie. Wszystkie wcześniejsze importery zostały uznane za wadliwe i nie wolno ich odtwarzać ani uruchamiać bez osobnej, wyraźnej zgody użytkownika. Poniższa instrukcja opisuje reguły importu, nie zgodę na użycie starego skryptu.
...
4. Ustal `playlist_id`, `playlist_series_id`, `service`, `name`, `source_file`, datę eksportu, a jeśli Maciek podał link także `external_playlist_id` i `external_url`. Tagi playlist zapisuj wyłącznie do `playlist.playlist_tags` jako zwykły `TEXT` rozdzielany średnikami. Dla playlist Maćka dodaj tag `owner:maciek-tora`.
...
8. Jeżeli dry-run jest czysty, uruchom ten sam jednorazowy kod z zapisem włączonym.
...
16. Legacy `playlist.tags` nie jest częścią obowiązującego modelu. Nie czytaj z niego, nie zapisuj do niego i nie używaj go jako fallbacku. Jedyną aktywną kolumną tagów playlist jest `playlist.playlist_tags`.

## Skrót

**plik → dry-run → czysty wynik → GitHub Actions → Turso COMMIT → live verify → usuń workflow.**

**Tagi: tylko `playlist.playlist_tags`, zwykły `TEXT`, średniki. `playlist.tags` = legacy.**
